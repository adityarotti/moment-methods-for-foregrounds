...
#   Fitting the Global monopole.
    def fit_mbb_monopole(self,nu,Inu,prange=2.,guess=[30.,0.,1e-6],lb=[0.,-3,0.],ub=[100.,3.,10.],maxfev=20000,makeplot=True):
        ...
        fit_param,fit_param_cov=curve_fit(self.mbb_moments,nu,Inu,p0=guess,bounds=(lb,ub),maxfev=maxfev)
...
#   Fitting functions
    ...
    def mbb_order1(self,nu,p0,p2,p3):
        ...
        temp=I0*(p0 + p2*(np.log(nu/nu0)) + p3*self.y1(x))
        return temp
...
#   Evaluating the cumulative error maps.
    ...
    def make_monopole_fit_plot(self):
        ...
        plt.plot(self.nu,self.mbb_moments(self.nu,T=par[0],alpha=par[1],A=par[2],p22=par[3],p23=par[4],p33=par[5],p222=par[6],p223=par[7],p233=par[8],p333=par[9]),"b--",lw=2,label="MBB fit")
        plt.plot(self.nu,abs(self.monopole-self.mbb_moments(self.nu,T=par[0],alpha=par[1],A=par[2],p22=par[3],p23=par[4],p33=par[5],p222=par[6],p223=par[7],p233=par[8],p333=par[9])),"r--",lw=2,label="Difference")
        plt.loglog()
        plt.xlabel("Frequency GHz")
        plt.ylabel(r"$I_{\nu}$")
        plt.legend(loc=0,fontsize=10)
        plt.ylim(1e-6,5e9)
        plt.yticks([1e7,1e5,1e3,1e1,1e-1,1e-2,1e-4,1e-6])
