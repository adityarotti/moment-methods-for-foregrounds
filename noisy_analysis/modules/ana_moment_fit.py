...
# Fitting carried out in the function defined below. Uses the scipy curve_fit routine.
    ...
    def fit_monopole_sed(self,nu,Inu,der_order=0,guess=[],lb=[],ub=[],err=[],prange=1000.,maxfev=20000,bounds_true=False,flat_sensitivity=True,verbose=False):
        ...
        if np.size(err)==0:
            if bounds_true:
                fit_par,fit_par_cov=curve_fit(self.moment_expansion_function,nu,Inu,p0=guess,sigma=err,absolute_sigma=False,bounds=(lb,ub),maxfev=maxfev)
            else:
                fit_par,fit_par_cov=curve_fit(self.moment_expansion_function,nu,Inu,p0=guess,sigma=err,absolute_sigma=False,maxfev=maxfev)
        else:
            if bounds_true:
                fit_par,fit_par_cov=curve_fit(self.moment_expansion_function,nu,Inu,p0=guess,sigma=err,absolute_sigma=True,bounds=(lb,ub),maxfev=maxfev)
            else:
                fit_par,fit_par_cov=curve_fit(self.moment_expansion_function,nu,Inu,p0=guess,sigma=err,absolute_sigma=True,maxfev=maxfev)
